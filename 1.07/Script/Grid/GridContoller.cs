...
public class GridContoller : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    private void SetUpEnemyGrid()
    {
        ...
        newEnemy.Init(5, 2, EquipmentInventory.Instance.GetEquipmentRandomlyByType(EquipmentType.Weapon));//前两个参数之后需要导入
    }
    ...
    private void ChangeToCardSide()
    {
        ...
        {
            CardManager.Instance.AddCard(CardInventory.Instance.GetCardRandomly());//随机添加一张卡
        }
    }
    ...
    public bool DoActionForEnemy(CardDifinition card) //被照亮才能执行,通过这个返回值判断操作是否可行,一般不会返回 false
    {
