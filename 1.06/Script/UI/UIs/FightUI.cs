...
public class FightUI : UIBase
{
    ...
    private List<CardDifinition> cardList; // 卡牌物体

    private void Awake()
    {
        cardList = new List<CardDifinition>();
    ...
    public void CreateCardItem(int count)
    {
        for (int i = 0; i < count; ++ i)
        {
            ...
            CardDifinition item = obj.AddComponent(Type.GetType("AttackCard")) as CardDifinition;

            item.Init(new CardBase());
            cardList.Add(item);
        }
    }
    ...
    public void RemoveCard(CardDifinition card)
    {
        ...
        // 添加到弃牌堆
        CardManager.Instance.usedCardList.AddCard(card);
